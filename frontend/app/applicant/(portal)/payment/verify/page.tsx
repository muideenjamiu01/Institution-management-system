...
export default function PaymentVerifyPage() {
  ...
  const getPaymentTypeMessage = () => {
    ...
    if (paymentInfo.type === 'APPLICATION_FEE') {
      return 'You can now proceed to fill your application form.';
    } else if (paymentInfo.type === 'ACCEPTANCE_FEE') {
      return 'Your matric number has been generated.';
    }
    return '';
  };
        ...
        <CardContent className="space-y-6">
          ...
          {status === 'success' && (
            <div className="space-y-6">
                ...
                <AlertDescription className="text-green-800">
                  <div className="space-y-2">
                    <p className="font-medium">
                      Your {paymentInfo?.type === 'APPLICATION_FEE' ? 'application' : 'acceptance'} fee payment has been confirmed.
                    </p>
                    <p>{getPaymentTypeMessage()}</p>
                    {paymentInfo?.reference && (
                      <p className="text-sm">Reference: {paymentInfo.reference}</p>
                    )}
                  </div>
                </AlertDescription>
              </Alert>
              ...
              <div className="flex gap-3">
                <Button onClick={() => router.push('/applicant/dashboard')} className="flex-1">
                  Go to Dashboard
                </Button>
                {paymentInfo?.type === 'APPLICATION_FEE' && (
                  <Button onClick={() => router.push('/applicant/application')} variant="outline" className="flex-1">
          ...
          {status === 'failed' && (
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                <Button onClick={() => router.push('/applicant/payment')} className="flex-1">
                  Try Again
                </Button>
                <Button onClick={() => router.push('/applicant/dashboard')} variant="outline" className="flex-1">
                  Go to Dashboard
                </Button>
              </div>
            </div>
